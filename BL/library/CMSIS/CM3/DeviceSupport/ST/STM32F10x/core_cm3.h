 ...
 *
 ******************************************************************************/
#include <stm32f10x.h>//miketsai add for swo debug
...
#define Core_SCS_BASE            (0xE000E000)                              /*!< System Control Space Base Address */
#define ITM_BASE            (0xE0000000)                              /*!< ITM Base Address                  */
#define CoreDebug_BASE      (0xE000EDF0)                              /*!< Core Debug Base Address           */
#define Core_SysTick_BASE        (Core_SCS_BASE +  0x0010)                      /*!< SysTick Base Address              */
#define Core_NVIC_BASE           (Core_SCS_BASE +  0x0100)                      /*!< NVIC Base Address                 */
#define Core_SCB_BASE            (Core_SCS_BASE +  0x0D00)                      /*!< System Control Block Base Address */

#define InterruptType       ((InterruptType_Type *) Core_SCS_BASE)         /*!< Interrupt Type Register           */
#define Core_SCB                 ((SCB_Type *)           Core_SCB_BASE)         /*!< SCB configuration struct          */
#define Core_SysTick             ((SysTick_Type *)       Core_SysTick_BASE)     /*!< SysTick configuration struct      */
#define Core_NVIC                ((NVIC_Type *)          Core_NVIC_BASE)        /*!< NVIC configuration struct         */
#define ITM                 ((ITM_Type *)           ITM_BASE)         /*!< ITM configuration struct          */
#define CoreDebug           ((CoreDebug_Type *)     CoreDebug_BASE)   /*!< Core Debug configuration struct   */

#if defined (__MPU_PRESENT) && (__MPU_PRESENT == 1)
  #define MPU_BASE          (Core_SCS_BASE +  0x0D90)                      /*!< Memory Protection Unit            */
...
static __INLINE void NVIC_SetPriorityGrouping(uint32_t PriorityGroup)
{
  ...
  reg_value  =  Core_SCB->AIRCR;                                                   /* read old register configuration    */
  reg_value &= ~(SCB_AIRCR_VECTKEY_Msk | SCB_AIRCR_PRIGROUP_Msk);             /* clear bits to change               */
  reg_value  =  (reg_value                       |
                (0x5FA << SCB_AIRCR_VECTKEY_Pos) | 
                (PriorityGroupTmp << 8));                                     /* Insert write key and priorty group */
  Core_SCB->AIRCR =  reg_value;
}
...
{
  return ((Core_SCB->AIRCR & SCB_AIRCR_PRIGROUP_Msk) >> SCB_AIRCR_PRIGROUP_Pos);   /* read priority grouping field */
}
...
{
  Core_NVIC->ISER[((uint32_t)(IRQn) >> 5)] = (1 << ((uint32_t)(IRQn) & 0x1F)); /* enable interrupt */
}
...
{
  Core_NVIC->ICER[((uint32_t)(IRQn) >> 5)] = (1 << ((uint32_t)(IRQn) & 0x1F)); /* disable interrupt */
}
...
{
  return((uint32_t) ((Core_NVIC->ISPR[(uint32_t)(IRQn) >> 5] & (1 << ((uint32_t)(IRQn) & 0x1F)))?1:0)); /* Return 1 if pending else 0 */
}
...
{
  Core_NVIC->ISPR[((uint32_t)(IRQn) >> 5)] = (1 << ((uint32_t)(IRQn) & 0x1F)); /* set interrupt pending */
}
...
{
  Core_NVIC->ICPR[((uint32_t)(IRQn) >> 5)] = (1 << ((uint32_t)(IRQn) & 0x1F)); /* Clear pending interrupt */
}
...
{
  return((uint32_t)((Core_NVIC->IABR[(uint32_t)(IRQn) >> 5] & (1 << ((uint32_t)(IRQn) & 0x1F)))?1:0)); /* Return 1 if active else 0 */
}
...
{
  if(IRQn < 0) {
    Core_SCB->SHP[((uint32_t)(IRQn) & 0xF)-4] = ((priority << (8 - __NVIC_PRIO_BITS)) & 0xff); } /* set Priority for Cortex-M3 System Interrupts */
  else {
    Core_NVIC->IP[(uint32_t)(IRQn)] = ((priority << (8 - __NVIC_PRIO_BITS)) & 0xff);    }        /* set Priority for device specific Interrupts  */
}
...
static __INLINE uint32_t NVIC_GetPriority(IRQn_Type IRQn)
{
  ...
  if(IRQn < 0) {
    return((uint32_t)(Core_SCB->SHP[((uint32_t)(IRQn) & 0xF)-4] >> (8 - __NVIC_PRIO_BITS)));  } /* get priority for Cortex-M3 system interrupts */
  else {
    return((uint32_t)(Core_NVIC->IP[(uint32_t)(IRQn)]           >> (8 - __NVIC_PRIO_BITS)));  } /* get priority for device specific interrupts  */
}
...
{ 
  ...
  Core_SysTick->LOAD  = (ticks & SysTick_LOAD_RELOAD_Msk) - 1;      /* set reload register */
  NVIC_SetPriority (SysTick_IRQn, (1<<__NVIC_PRIO_BITS) - 1);  /* set Priority for Cortex-M0 System Interrupts */
  Core_SysTick->VAL   = 0;                                          /* Load the SysTick Counter Value */
  Core_SysTick->CTRL  = SysTick_CTRL_CLKSOURCE_Msk | 
...
{
  Core_SCB->AIRCR  = ((0x5FA << SCB_AIRCR_VECTKEY_Pos)      | 
                 (Core_SCB->AIRCR & SCB_AIRCR_PRIGROUP_Msk) | 
